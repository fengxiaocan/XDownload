...
final class SingleDownloadM3u8Task extends HttpDownloadRequest implements IDownloadRequest, IConnectRequest {
        ...
        try {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                try {
                    ...
                    com.x.down.tool.XDownUtils.closeIo(inputStream);
                }
            }
        } finally {
            com.x.down.tool.XDownUtils.closeIo(outputStream);
        }
    }
    ...
    protected void httpRequest() throws Exception {
        //判断之前下载的文件是否存在或完成
        File tempCacheDir = com.x.down.tool.XDownUtils.getTempCacheDir(request);
        ...
        for (int i = 0; i < m3U8DownloaderBlocks.size(); i++) {
            ...
            if (start > 0) {
                http.setRequestProperty("Range", com.x.down.tool.XDownUtils.jsonString("bytes=", start, "-", length));
            }
    ...
    private long downloadLong(M3u8DownloaderBlock block) throws Exception {
        ...
        //优先获取文件长度再回调
        long contentLength = com.x.down.tool.XDownUtils.getContentLength(http);
        //连接中
        listenerDisposer.onConnecting(this);

        if (contentLength <= 0) {
            //长度获取不到的时候重新连接 获取不到长度则要求http请求不要gzip压缩
            com.x.down.tool.XDownUtils.disconnectHttp(http);
            http = request.buildConnect();
            http.setRequestProperty("Accept-Encoding", "identity");
            http.connect();

            contentLength = com.x.down.tool.XDownUtils.getContentLength(http);
            //连接中
        }
        listenerDisposer.onConnecting(this);
    ...
    private void onResponseError(HttpURLConnection http, int responseCode) throws IOException {
        String stream = readStringStream(http.getErrorStream(), com.x.down.tool.XDownUtils.getInputCharset(http));
