...
public class XDownloadRequest extends BaseRequest implements HttpDownload, BuilderURLConnection {
    ...
    //默认下载的多线程数
    protected int multiThreadCount = Config.config().getSameTimeDownloadMaxCount();
    //默认多线程下载的单线程最大下载文件块大小,默认10MB
    protected int multiThreadMaxDownloadSize = Config.config().getMultiThreadMaxDownloadSize();
    //默认多线程下载的单线程最小下载文件块大小,默认100KB
    protected int multiThreadMinDownloadSize = Config.config().getMultiThreadMinDownloadSize();
    //是否使用多线程下载
